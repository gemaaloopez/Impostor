		...
		io.on('connection',function(socket){		    
		  ...
		  //   	var datos={nick:nick,numJugador:numero};
			...
			socket.on("votar",function(nick,codigo,sospechoso){
		    	...
		    	if (partida.todosHanVotado()){
		    		var data={"elegido":partida.elegido,"fase":partida.fase.nombre};
		    		console.log('servidorWS votar', partida.fase.nombre)
			    	if (partida.fase.nombre=="final"){
				    	cli.enviarATodos(io, codigo, "final", partida.ganador);
				    } 
				    else{
				    	cli.enviarATodos(io, codigo,"finalVotacion", data);
				    }
		    ...
		    socket.on("atacar",function(nick,codigo,inocente){
			    ...
			    if (fase=="final"){
			    	cli.enviarATodos(io, codigo, "final","Impostores");
			    }
		    });
		    ...
		    socket.on("realizarTarea",function(nick,codigo){
		    	...
		    	var percentGlobal=partida.obtenerPercentGlobal();
				cli.enviarRemitente(socket,"tareaRealizada",{"percent":percent,"global":percentGlobal});
				cli.enviarATodos(io,codigo,"tareaGlobalRealizada",percentGlobal);		    	
		    	var fase=partida.fase.nombre;
		    	if (fase=="final"){
			    	cli.enviarATodos(io, codigo, "final","Ciudadanos");
			    }
		    });
