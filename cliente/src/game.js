...
function lanzarJuego(){
  ...
  var crear;
  var spawnPoint;
  var recursos=[{frame:0,sprite:"chumi"},{frame:3,sprite:"gema"},{frame:6,sprite:"pepe"},{frame:9,sprite:"pablo"},{frame:48,sprite:"paco"},{frame:51,sprint:"palo"},{frame:54,sprint:"pedro"},{frame:57,sprint:"antonio"}];;
  var remotos;
  var muertos;
  var capaTareas;
  var tareasOn=true;
  ...
  var mostrarNombre;
  var mostrarNombreRemoto=[];
  //var followTextRemotoMuerto;
  var mostrarEncargo;
  //var textImpostor;
  var mostrarProgreso;
  var mostrarProgresoGlobal;
  ...
  function preload() {
    ...
    this.load.spritesheet("varios","cliente/assets/images/personajes.png",{frameWidth:32,frameHeight:32});
  ...
  function dibujarMuereInocente(inocente){
    ...
    if(ws.nick != inocente){
      ...
      mostrarNombreRemoto[numJugador].setActive(false).setVisible(false);
    }

    var muerto = crear.physics.add.sprite(x,y,"muertos",recursos[numJugador].frame);
    muertos.add(muerto);

    //crear.physics.remove.sprite(x,y,"varios",recursos[numJugador].frame)
  ...
  function tareas(sprite,objeto){
    if (ws.encargo==objeto.properties.tareas && Phaser.Input.Keyboard.JustDown(teclaT)/*teclaT.isDown*/){      
      console.log("realizar tarea "+ws.encargo);
      ws.realizarTarea(); //o hacer la llamada dentro de cw
      if(ws.tareaPercent==100){
        tareasOn=false;
      }
    }
  }

  function actualizarTareas(percent, percentGlobal){
    mostrarProgresoGlobal.setText("Progreso Global: "+percentGlobal+"%");
    if(!ws.impostor){
      mostrarProgreso.setText("Progreso Local: "+percent+"%");
      cw.mostrarModalTarea("Tarea realizada: "+ws.encargo+". Progreso: "+percent+"%");
      if(percent>=100){
        tareasOn=false;
      }
    }
  ...
  function actualizarTareasGlobales(percentGlobal){
    mostrarProgresoGlobal.setText("Progreso Global: "+percentGlobal+"%");
  }

  function jugadorAbandonaPartida(inocente){
    if(ws.nick != inocente){
      jugadores[inocente].setActive(false).setVisible(false);
      mostrarNombreRemoto[jugadores[inocente].numJugador].setActive(false).setVisible(false);
    }
  }

  function jugadorVotado(inocente){
    if(ws.nick != inocente){
      jugadores[inocente].setActive(false).setVisible(false);
      mostrarNombreRemoto[jugadores[inocente].numJugador].setActive(false).setVisible(false);
    }
  ...
  function lanzarJugador(nick,numJugador){
    ...
    this.mostrarNombre = crear.add.text(0, 0, jugadores[nick].nick);
    mostrarNombre.setDepth(13);

    encargo = ws.encargo
    if(ws.impostor){
      encargo = "impostor";
    }   
    this.mostrarEncargo = crear.add.text(10, 0, "Encargo: " + encargo);
    mostrarEncargo.setStyle({
      fontSize: '16px',
      fontFamily: 'Comic Sans',
      color: '#FF00FF'
    });
    mostrarEncargo.setDepth(13);

    if(!ws.impostor){
      this.mostrarProgreso = crear.add.text(50, 30, 'Progreso: ' + 0 + '%');
      mostrarProgreso.setStyle({
        fontSize: '10px',
        fontFamily: 'Comic Sans',
        color: '#FF00FF'
      });
      mostrarProgreso.setDepth(13);
    }

    this.mostrarProgresoGlobal = crear.add.text(50, 30, 'Progreso Global: ' + 0 + '%');
    mostrarProgresoGlobal.setStyle({
      fontSize: '10px',
      fontFamily: 'Comic Sans',
      color: '#FF00FF'
    });
    mostrarProgresoGlobal.setDepth(13);
  }
  ...
  function lanzarJugadorRemoto(nick,numJugador){
    ...
    this.mostrarNombreRemoto[numJugador] = crear.add.text(0, 0, jugadores[nick].nick);
    this.mostrarNombreRemoto[numJugador].setDepth(13);
  }
  ...
  function mover(datos)
  {
    ...
    this.mostrarNombreRemoto[numJugador].setPosition(remoto.x-30, remoto.y-40);
  }
  ...
  function update(time, delta) {
    ...
    if (!final){
      ...
      mostrarNombre.setPosition(player.x-30, player.y-40);
      mostrarEncargo.setPosition(camera.worldView.x+295, camera.worldView.y);

      if(!ws.impostor){
        mostrarProgreso.setPosition(camera.worldView.x+287, camera.worldView.y+15);
      }
      mostrarProgresoGlobal.setPosition(camera.worldView.x+287, camera.worldView.y+25);
