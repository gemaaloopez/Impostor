function ClienteWS(){
	...
	this.lanzarSocketSrv=function(){
		...
		this.socket.on("votacion",function(lista){
			console.log(lista);
			cw.mostrarModalVotacion(lista, cli.estado);
		});
		...
		this.socket.on("recibirEncargo",function(data){
			console.log("encargo:" + JSON.stringify(data));
			cli.impostor=data.impostor;
			cli.encargo=data.encargo;
			if (data.impostor){
				//$('#avisarImpostor').modal("show");
				cw.mostrarModalSimple('Ahora tú, eres el impostor!!');
				//crearColision();
			}
		});
		...
		this.socket.on("tareaRealizada",function(data){
			cli.tareaPercent=data.percent;
			cli.globalPercent=data.global;
			actualizarTareas(data.percent, data.global);
			//tareasOn=true;
		});

		this.socket.on("tareaGlobalRealizada",function(data){
